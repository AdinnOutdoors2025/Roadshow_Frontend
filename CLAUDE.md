...
## Established UI/UX patterns (public roadshow site)

- **Image loading placeholder**: reserve space with CSS `aspect-ratio` on the image wrapper (e.g. `.HomeBannerMainImgWrapper { aspect-ratio: 4096 / 1446; }` in `HomeBanner.css`) rather than a colored/boxy placeholder `<div>`. This guarantees correct layout at first paint regardless of network speed or cache state (no JS/timing dependency), matching this site's existing "no boxy placeholder" design intent.
- **Nav link hover "odometer" roll effect** (`src/components/Client/Reusable_Components/Navbar.tsx`): each nav label renders two stacked copies of its text inside an `overflow:hidden` mask (`.RS_RefNavLabel` > two `.RS_RefNavLabelText` spans, the second `--hover` one pre-positioned off-screen via `translate3d`), and hover/focus swaps their `translate3d` offsets so the base text rolls out while the red hover copy rolls in. All nav links behave identically (black at rest, rolls to red on hover) — there is intentionally **no** special-cased styling for the active tab (`.RS_RefNavLink--active` carries the class in JSX for potential future use but has no CSS rule). Don't reintroduce active-tab color overrides without confirming with the user first; a prior version of this had one and it was explicitly removed.
- **Entrance-animation gating on the initial loader**: components with mount-triggered entrance animations (e.g. `HomeBanner.tsx`'s heading/image/thumbnail reveals) should not animate immediately on mount if a full-page loader may still be covering them. Gate them behind `MAIN_LOADER_DONE_EVENT` / `hasMainLoaderCompletedOnce()` (exported from `src/components/GlobalRoadshowLoader.tsx`), with a short fallback timeout (see `READY_FALLBACK_MS` in `HomeBanner.tsx`) in case the loader event never fires. `SplitHeading` (`src/components/motion/SplitHeading.tsx`) exposes an `enabled` prop (default `true`) for exactly this — pass `false` until the loader clears.
- **File preview vs download** (`src/components/ui/FilePreviewModal.tsx`): never preview a Space/backend file URL directly in `<img>`/`<iframe>`/`target="_blank"` — Space files often come back with `Content-Disposition: attachment` or `application/octet-stream`, which forces a download instead of an inline preview. Instead `fetch()` the file, rebuild its `Blob` with the correct MIME type guessed from the filename/label (`guessMime`), and preview via `URL.createObjectURL`. Keep an explicit separate "Download" action using the original URL. `isImageFile`/`guessMime` fall back to checking the `label` (real filename) when the `url` itself has no extension (e.g. a local `blob:` URL for a not-yet-uploaded file).
- Layout-shake prevention: `scrollbar-gutter: stable` is set on `html` in `src/app/globals.css` so pages with varying content height don't shift width when the vertical scrollbar appears/disappears. Don't remove this to "fix" an unrelated scroll issue without checking why it's there.

## Backend contract notes relevant to this (frontend-only) repo

These live in the separate backend repo but affect what this frontend can assume from API responses — useful context since this repo has no backend code of its own to consult directly:

- **Admin order edits sync to client-facing reads at read time, not write time.** When admin edits an `Order` linked to a `ClientRequest` (dates, vehicle model, quantity, pricing, or adds/removes a vehicle line), the backend overlays the Order's live `bookingItems` onto the ClientRequest's response in `Utils/clientJourneyStages.js` (`applyOrderFieldOverrides` / `applyOrderTotalsOverride`) every time `client-requests/mine`, `client-requests/:id`, or the tracking endpoint is read — the ClientRequest document itself is never rewritten. This frontend's `my-bookings` pages and PDF summary generation already do a fresh `GET` before rendering, so they pick up admin edits automatically with no frontend change required.
- **Deleting a `ClientRequest` cascades**: the backend also deletes its linked `Order` (if any) and cleans up every associated file in DigitalOcean Space (campaign images/videos, PO document, any Order-side media) via `deleteManyFromSpaces`/`collectSpaceUrls`. There is currently no admin UI in this repo for deleting a client request — if one is added, expect this cascade behavior and don't add frontend-side "are you sure this also deletes the order" messaging without confirming current backend behavior first.
