...
## 12. Compensation & Extra KM/Hours — released/replaced vehicles must not keep collecting (fixed this session)

**Business rule:** a released/replaced vehicle (`entryId`) is "completed" as of its release day (the `changedAt` date of its `onRoadDriverHistory` `action: "removed"` event). Slot-level (broadcast, `entryId: null`) grants and records are a *position* entitlement — they follow whichever vehicle is genuinely active in the slot that day (old vehicle while active, replacement once it takes over), never a specific vehicle permanently. Entry-specific grants/records (tied to one exact `entryId`) apply only through that vehicle's own release day — never after.

**Compensation fix:** `compensationHoursFor(entryId, vehicleIndex, dayKeyStr, isActiveToday)` (~line 3672) gained an `isActiveToday` parameter — entry-specific grants (`hourCompByEntry`) always apply, but the slot-level/broadcast fallback (`hourCompByVehicle`) is now skipped entirely when `isActiveToday` is false. `computeEntryDayFigures(...)` (~line 3801) forwards this: `true` for entries in the `activeEntries` branch, `false` for entries in the `releasedToday` branch (~line 3904-3921) — so a vehicle shown as "Released Today" no longer picks up that day's slot-level compensation banner; only the currently-active entries (including a same-day replacement) do. Entry-specific grants for the released vehicle still apply through its release day (unaffected — the surrounding loop already stops calling this function for an entry on any day after its own release, so no separate clipping was needed there).

**Extra KM/Hours fix:** unlike compensation, extra-km resolution runs independently of the active/released entry loop, so it had no equivalent protection. Added `clipExtraKmRecordsToEntryLifetime(records)` (~line 3703, needs `historyByEntry` in scope) — for any entry-specific effective record (`entryId` set), if that entry was ever removed, its resolved `toDate` is clamped to the entry's release day, even if the original record's `toDate` extends further (e.g. a record logged 29→31 Jul for a vehicle released on 30 Jul now only applies through 30 Jul; 31 Jul gets zero for that vehicle). Campaign-level records (`entryId: null`) aren't tied to one vehicle and are left untouched. Applied by wrapping both `resolveEffectiveExtraKmRecords(...)` call sites (`extraKmBalanceByVehicle` ~line 3773, `effectiveExtraKmRecordsByVehicle` ~line 3891).
