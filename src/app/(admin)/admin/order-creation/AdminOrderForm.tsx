...
export default function AdminOrderForm({ onClose, onSuccess }: Props) {
  ...
  // Display name in header — individual: name, organization: clientName
  const displayName =
            ...
            {STEPS.map((s, i) => {
              ...
              return (
                <React.Fragment key={i}>
                  ...
                  {i < STEPS.length - 1 && (
                    <div
                      className={`flex-1 h-0.5 mx-1 mb-4 rounded transition-all ${
          ...
          {step === 0 && (
            <CustomerDetailsStep
              ...
              onCategoryChange={(cat) =>
                setOrder((p) => ({ ...p, customerCategory: cat }))
              }
            />
          )}
          {step === 1 && (
