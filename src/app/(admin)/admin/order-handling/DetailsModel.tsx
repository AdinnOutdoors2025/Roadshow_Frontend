...
const STAGE_MAP: Record<string, { label: string; gradient: string; icon: string }> = {
    ...
    customerConfirmation: { label: "Customer Confirmation", gradient: "from-teal-400 to-emerald-500", icon: "✅" },
                ...
                <div className="flex-1 overflow-y-auto p-3 md:p-6 space-y-3 md:space-y-5 bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-800">
                    ...
                    <Section icon="👤" title="Customer Information" accent="from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-3">
                            ...
                            <InfoChip icon={<Phone size={14} />} label="Phone" value={order.phone} />
                    ...
                    {order.negotiationLogs && order.negotiationLogs.length > 0 && (
                        <Section icon="💬" title="Negotiation History" accent="from-amber-50 to-yellow-50 dark:from-amber-900/20 dark:to-yellow-900/20">
                            <div className="space-y-2 md:space-y-3">
                                {order.negotiationLogs
                                    .filter((log) => (log.discountAmount || 0) > 0)
                                    .map((log, i) => (
                                        <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-3 md:p-4 rounded-lg md:rounded-xl bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800/40 dark:to-gray-800/60 border border-gray-200 dark:border-gray-700/50 hover:shadow-md transition-all group">
                                            <div className="flex items-center gap-2 md:gap-3">
                                                <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white font-bold text-xs md:text-sm shadow-lg">
                                                    {i + 1}
                                                </div>
                                                <div>
                                                    <p className="text-md md:text-md font-semibold text-gray-700 dark:text-gray-300">
                                                        {log.fromStage} → {log.toStage}
                                                    </p>
                                                    <p className="text-[10px] md:text-xs text-gray-400 mt-0.5 md:mt-1">
                                                        {log.movedBy} · {fmtDatetime(log.movedAt)}
                                                    </p>
                                                </div>
                                            </div>
                                            <div className="sm:text-right pl-10 sm:pl-0">
                                                <p className="text-[10px] md:text-xs text-gray-500 mb-0.5 md:mb-1">Discount</p>
                                ...
                                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 p-3 md:p-4 mt-2 md:mt-3 bg-gradient-to-r from-gray-800 to-gray-700 dark:from-gray-700 dark:to-gray-600 rounded-lg md:rounded-xl text-white">
                                    <span className="text-xs md:text-sm font-semibold">Final Negotiated Amount</span>
                    ...
                    <Section icon="🔄" title="Pipeline Journey" accent="from-violet-50 to-purple-50 dark:from-violet-900/20 dark:to-purple-900/20">
                        <div className="space-y-0">
                            {[...order.pipelineLogs].reverse().map((log, i, arr) => {
                                ...
                                return (
                                    <TimelineItem key={i} dotColor={toStage?.gradient || 'from-gray-400 to-gray-500'} isLast={isLast}>
                                        <div className="bg-gray-50 dark:bg-gray-800/40 rounded-lg md:rounded-xl p-2.5 md:p-3.5 border border-gray-100 dark:border-gray-700/50 hover:shadow-md transition-all">
                                            <p className="text-md md:text-md font-semibold text-gray-800 dark:text-gray-200">
                                                {log.fromStage
                                            ...
                                            {log.handlerName && (
                                                <div className="flex items-center gap-1 md:gap-1.5 mt-1.5 md:mt-2">
                                                    <div className="w-4 h-4 md:w-5 md:h-5 rounded-full bg-violet-100 dark:bg-violet-900/30 flex items-center justify-center text-[10px] md:text-xs">
                                                        👤
                                                    </div>
                                                    <p className="text-[10px] md:text-xs text-violet-600 dark:text-violet-400 font-medium">{log.handlerName}</p>
                                                </div>
                                            )}
                                            <div className="flex items-center gap-1.5 md:gap-2 mt-1.5 md:mt-2 text-[12px] md:text-xs text-gray-400 flex-wrap">
                                ...
                                <div className="flex-1">
                                    ...
                                    {totalAdvance > 0 && (
                                        <div className="mt-2 pt-2 border-t border-green-200 dark:border-green-700">
                                            <p className="text-[10px] md:text-xs text-orange-600 dark:text-orange-400 font-semibold uppercase tracking-wider mb-0.5">Balance Due</p>
                                            <p className="text-base md:text-lg font-bold text-orange-600 dark:text-orange-400">{fmt(balanceAmount)}</p>
                                        </div>
                                    )}
                                </div>
                                <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-green-500 flex items-center justify-center text-xl md:text-2xl shadow-lg">
