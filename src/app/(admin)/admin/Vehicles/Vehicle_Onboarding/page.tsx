...
//     WaitingforStatus: "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400",
//   };
//   return (
//     <span
//       className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${styles[status] || styles.WaitingforStatus
//         }`}
//     >
//       {status}
//     </span>
...
//       ...AdminSelectOptions.permitOptions.map((option) => ({ value: option.value, label: option.label })),
//     ],
//     modelOptions: [
// ...AdminSelectOptions.modelOptions.map((option) => ({ value: option.value, label: option.label })),
//     ],
//     ownershipOptions: [
//       ...AdminSelectOptions.ownershipOptions.map((option) => ({ value: option.value, label: option.label })),
//     ],
//     fuelTypeOptions: [
//       ...AdminSelectOptions.fuelTypeOptions.map((option) => ({ value: option.value, label: option.label })),
//     ],
//     // currentStatusOptions: [
//     //   ...AdminSelectOptions.currentStatusOptions.map((option) => ({ value: option.value, label: option.label })),
//     // ],
//   };
...
//                       </Label>
//                      <Select
//   options={AdminSelectOptions.remarks.map((r) => ({ value: r, label: r }))}
...
//   ...AdminSelectOptions,                          // spread all static options from JSON
//   vehicleTypeOptions: vehicleTypes.map((vt) => ({ // only this stays dynamic (comes from API)
//     value: vt._id,
//     label: vt.typeName,
//   })),
// });
//   const selectOptions = getSelectOptions();
// console.log("Select options with dynamic vehicle types:", selectOptions);
...
// //     saveCurrentStep(1, null);
// //   }
// // }, [vehicles]); // fires whenever vehicles array changes
...
//       // const typeName = vehicleTypes.find(t => t._id === data.basicInfo.vehicleType)?.typeName || data.basicInfo.vehicleType;
//       // After: setCommonInfo(data.basicInfo);
...
//         autoClose: 3000,
//       });
//        // NEW: auto-save step 1 after adding vehicle
//   // if (commonInfo.vehicleType) {
//   //   await saveCurrentStep(1, null);  // null = don't navigate
//   // }

//   //  if (currentStep === 1 && currentEditingGroupId) {
//   //   await saveCurrentStep(1, 1);
//   // } else if (currentStep === 1 && !currentEditingGroupId) {
//   //   // First time – create group via saveCurrentStep
//   //   await saveCurrentStep(1, 1);
//   // }

// //   // Replace the auto-save block at bottom of handleAddVehicle with:
// // if (currentStep === 1) {
// //   // Small delay to ensure vehicles state is updated before saving
// //   setTimeout(async () => {
// //     await saveCurrentStep(1, null);
// //   }, 100);
// // }
//     }
//     setEditingVehicle(null);
...
//   setVehicles((prev) =>
//     prev.filter(v => unformatRegistrationNumber(v.registrationNumber) !== unformatRegistrationNumber(registrationNumber))
//   );
// };
...
//     ...prev,
//     vehicleType: value,
//     vehicleName: typeName,  // ← auto-set from typeName
//   }));
// }}
//                     />
//                     <span className="absolute text-gray-500 -translate-y-1/2 pointer-events-none right-3 top-1/2 dark:text-gray-400">
...
//           {currentStep === 4 && (
//             <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
//               <SectionHeader number={4} title="Vehicle Summary" icon="📊" />
...
//                 //     : `✅ Submit ${vehicles.length} Vehicle(s)`}
//                 // </button>
//                 <button type="submit" disabled={loading || !stepCompletionStatus[3]} className="px-6 py-2.5 bg-green-600 text-white rounded-lg">
//                   {loading ? "Submitting..." : `Submit ${vehicles.length} Vehicle(s)`}
//                 </button>
//               )}
//             </div>
